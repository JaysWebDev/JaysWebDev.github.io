...
# Universe sourced from DB (all active symbols) — not a watchlist file
# Previously used my_main_512.txt (only 514 symbols); now covers full 2,700+ universe
...
def load_watchlist():
    """Return all active symbols from DB (single source of truth)."""
    try:
        ...
        print(f"Warning: could not load symbols from DB: {e}")
        return set()
